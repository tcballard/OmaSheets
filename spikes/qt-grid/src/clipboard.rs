...
pub fn parse(text: &str) -> Result<Vec<Vec<String>>, String> {
    if text.len() > MAX_BYTES {
        return Err("clipboard exceeds 1 MiB".into());
    }
    ...
    while let Some(ch) = chars.next() {
        ...
        if ch == '\t' || ch == '\n' || ch == '\r' {
            ...
            count += 1;
            if count > MAX_CELLS {
                return Err("clipboard exceeds 1,000 cells".into());
            }
            closed = false;
            if ch != '\t' {
    ...
    if quoted {
        return Err("clipboard contains an unfinished quoted field".into());
    }
    if !ended_row {
        row.push(field);
        count += 1;
...
mod tests {
    ...
    fn copied_formulas_shift_relative_axes_only() {
        assert_eq!(
            translate_formula("=A1+$B2+C$3+$D$4+SUM(A1:B2)", 2, 3),
            "=D3+$B4+F$3+$D$4+SUM(D3:E4)"
        );
        assert_eq!(
            translate_formula("='Bob''s A1'!B2+A1!C3+LOG10(A1)+1E10+Tax.A1+名A1", 1, 1),
            "='Bob''s A1'!C3+A1!D4+LOG10(B2)+1E10+Tax.A1+名A1"
        );
        assert_eq!(
            translate_formula(
                "=IF(A1=\"A1\",\"say \"\"B2\"\"\",T[[#All],[A1]])+A1[B2]",
                1,
                1
            ),
        ...
        assert_eq!(
            translate_formula("=A1+$A1+A$1+$A$1", -1, -1),
            "=#REF!+#REF!+#REF!+$A$1"
        );
        assert_eq!(
            translate_formula("=XFD1048576+XFE1+A1048577", 1, 1),
            "=#REF!+XFE1+A1048577"
        );
        assert_eq!(translate_formula("=B2:C3", -1, -1), "=A1:B2");
        let mut rows = vec![
            vec!["=A1".into(), "'=A1".into(), "A1".into()],
            vec!["=A2".into()],
        ];
    ...
    fn quoted_clipboards_round_trip() {
        ...
        assert_eq!(
            parse(&encode(&trailing_blank).unwrap()).unwrap(),
            trailing_blank
        );
        assert_eq!(parse("a\tb\r\nc\td\r\n").unwrap().len(), 2);
